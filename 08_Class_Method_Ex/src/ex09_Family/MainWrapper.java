package ex09_Family;

public class MainWrapper {

  public static void ex01() {
    Family f = new Family(3);
    
    String[] name = {"나", "엄마", "아빠"};
    int[] age = {20, 40, 43};
    
    for(int i = 0; i < f.getFamily().length; i++) {
      f.getFamily()[i] = new Person();    // 디폴트 생성자를 이용해서 배열요소를 저장해 둔다.
    
      
      // setter를 이용해 이름과 나이 저장.
      f.getFamily()[i].setName(name[i]);
      f.getFamily()[i].setAge(age[i]);  
    }
    for(int i = 0; i < f.getFamily().length; i++) {
      System.out.println((i + 1) + "번째 가족 이름: " + f.getFamily()[i].getName());
      System.out.println((i + 1) + "번째 가족 나이: " + f.getFamily()[i].getAge());
    }
    
  }
  
  public static void ex02() {
    Family f = new Family(3);
    
    String[] name = {"나", "엄마", "아빠"};
    int[] age = {20, 40, 43};
    
    // 파라미터가 2개인 생성자를 이용
    for(int i = 0; i < f.getFamily().length; i++) {
      f.getFamily()[i] = new Person(name[i], age[i]);
    }
    
    for(int i = 0; i < f.getFamily().length; i++) {
      System.out.println((i + 1) + "번째 가족 이름: " + f.getFamily()[i].getName());
      System.out.println((i + 1) + "번째 가족 나이: " + f.getFamily()[i].getAge());
    }
   }
  public static void ex03() {
    Family f = new Family(3);
    
    String[] name = {"나", "엄마", "아빠"};
    int[] age = {20, 40, 43};
    
    Person[] family = f.getFamily();
    
    for(int i = 0; i < f.getFamily().length; i++) {
      family[i] = new Person();
      family[i].setName(name[i]);
      family[i].setAge(age[i]);
    }
    
    for(Person p : family) {
      System.out.println("가족 이름: " + p.getName());
      System.out.println("가족 나이: " + p.getAge());
    }
    
  }
  
  public static void ex04() {
    Family f = new Family(3);
    
    String[] name = {"나", "엄마", "아빠"};
    int[] age = {20, 40, 43};
    
    Person[] family = f.getFamily();
    
    for(int i = 0; i < f.getFamily().length; i++) {
      family[i] = new Person(name[i], age[i]);
    }
    for(Person p : family) {
      if(p != null) {                                   // NullPointerException오류 때문에 넣는게 좋다.
      System.out.println("가족 이름: " + p.getName());
      System.out.println("가족 나이: " + p.getAge());   // 참조하는 객체가 null이 되면 오류가 난다. NullPointerException
    
      }
    }
  }
  
  
  public static void main(String[] args) {
//    ex01();
//    ex02();
//    ex03();
    ex04();
    
  }

}
